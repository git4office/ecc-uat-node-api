    ...
    query = "SELECT * FROM ["+dbName+"].ECCAnalytics.Users where username ='"+username+ "'"
     request.query(query,function(err,records){
         ...
         return res.status(200).json('User exists')
          ...
          query2 = "INSERT INTO ["+dbName+"].ECCAnalytics.Users (username, roles,useremailid,pswd) VALUES ('"+username+"','"+roles+"','"+useremailid+"','"+pswd+"')"
...
const postdatapoint = (req,res)=>{
  ...
  sql.connect(config,function(err){
     ...
     request.query(query,function(err,records){
      ...
      return res.status(200).json({status:'data exists'})
